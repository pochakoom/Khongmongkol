...
class WidFlower extends StatelessWidget {
  final Item item;
  final String day;

  const WidFlower({Key? key, required this.item, required this.day})
      : super(key: key);
  ...
  Widget _buildBodyContent() {
    List<Item> filteredItems = itemData
        .where((item) =>
            (item.category == 'plant1' ||
             item.category == 'plant2' ||
             item.category == 'plant3') &&
             item.day == day)
        .toList();
